...
- **Go (security baseline):** использовать патч-версию `>=1.25.10`.
  Для локальных запусков можно явно задавать:
  `GO_TOOLCHAIN=go1.25.10 bash ./scripts/go_test_backend.sh ./...`
...
```bash
# API
cd backend && GOTOOLCHAIN=go1.25.10 go run ./cmd/api
