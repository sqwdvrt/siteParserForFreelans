...
const baseURL = "https://www.weblancer.net"
const baseHost = "weblancer.net"

// Extractor парсит HTML страниц Weblancer.net.
type Extractor struct{}

// NewExtractor создаёт экстрактор для Weblancer.
func NewExtractor() *Extractor {
	return &Extractor{}
}

// ExtractList извлекает URL проектов из HTML страницы со списком Weblancer.
func (e *Extractor) ExtractList(html []byte) ([]string, error) {
	...
	// Weblancer: ссылки на проекты вида /jobs/category/ID-slug/
...
// ExtractDetail извлекает Job из HTML страницы проекта Weblancer.
...
// resolveURL превращает относительный href в абсолютный URL weblancer.net.
...
// projectDetailRe: /jobs/<категория>/<числовой-id>
var projectDetailRe = regexp.MustCompile(`/jobs/[^/]+/\d+`)

var externalIDRe = regexp.MustCompile(`/jobs/[^/]+/(\d+)`)
