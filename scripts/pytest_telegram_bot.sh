...
if [[ "${PYTEST_TELEGRAM_BOT_AUTO_INSTALL:-0}" == "1" ]]; then
  "${ROOT_DIR}/scripts/bootstrap_telegram_bot_venv.sh"
  exec "${BOT_DIR}/.venv/bin/python" -m pytest "$@"
fi

echo "ERROR: pytest is unavailable for telegram-bot." >&2
echo "Bootstrap a local venv once:" >&2
echo "  ./scripts/bootstrap_telegram_bot_venv.sh" >&2
echo "Or run with an explicit interpreter that already has pytest:" >&2
echo "  PYTHON_BIN=python3.11 ./scripts/pytest_telegram_bot.sh -q" >&2
exit 1
